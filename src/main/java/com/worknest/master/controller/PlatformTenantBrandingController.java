...
public class PlatformTenantBrandingController {
    ...
    public ResponseEntity<ApiResponse<TenantBrandingViewDto>> uploadLogo(
            ...
            @RequestHeader(value = HttpHeaders.IF_MATCH, required = false) String ifMatch) {
        return writeResponse("Company logo updated successfully", tenantBrandingService.uploadPlatformLogo(
                tenantKey,
                file,
                BrandingHttpSupport.parseVersion(ifMatch)
        ));
    ...
    public ResponseEntity<ApiResponse<TenantBrandingViewDto>> deleteLogo(
            @PathVariable("tenantKey") String tenantKey,
            @RequestHeader(value = HttpHeaders.IF_MATCH, required = false) String ifMatch) {
        return writeResponse("Company logo removed successfully", tenantBrandingService.deletePlatformLogo(
                tenantKey,
                BrandingHttpSupport.parseVersion(ifMatch)
        ));
    }
